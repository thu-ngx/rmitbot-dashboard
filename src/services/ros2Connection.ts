...
class RosService {
  ...
  public publishVelocity(
    ...
    if (!this.isConnected || !this.cmdVelPublisher) {
      console.warn(
        "Cannot publish velocity - not connected or publisher not initialized",
      );
      return;
    }
    ...
    try {
      ...
      console.log(
        `Published velocity - linear: [${linear_x.toFixed(2)}, ${linear_y.toFixed(2)}, 0], angular: [0, 0, ${angular_z.toFixed(2)}]`,
      );
    } catch (error) {
  ...
  switchRobot(newIp: string) {
    ...
    useConnectionStore.getState().setCurrentIp(newIp);
